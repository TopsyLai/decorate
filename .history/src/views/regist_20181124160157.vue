<template>
    <div class="regist">
        <group>
            <x-input title="" ref="phone" name="phone" placeholder="请输入手机号" v-model="phone" keyboard="number" is-type="china-mobile" required></x-input>
            <x-input title="" ref="verifyCode" max="4" min="4" type="text" placeholder="请输入图片验证码" v-model="verifyCode" required>
              <img slot="right-full-height" :src="getCode.img" />
            </x-input>
            <x-input title="" type="text" max=4 min=4 placeholder="请输入短信验证码" v-model="mobileCode	" required>
  ...
  methods: {
    handleGetCode() {
      if (this.phone) {
        ...
        let self = this;
        this.$post("account/login.htm", params).then(function(json){
          console.log('成功')
          console.log(json)
